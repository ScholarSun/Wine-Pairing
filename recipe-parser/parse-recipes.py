...
extras = ['carnations', 'coloring', 'dust', 'flowers', 'lilies', 'spray', 'toppings', 'drippings', 'powdered', 'gold']
fasteners = ['sticks', 'skewers', 'toothpicks']
adhesives = ['glue']
containers = ['jars']
flavorings = ['extract', 'flavorings', 'mint', 'pandan', 'hickory', 'flavored', 'mesquite', 'wood', 'hardwood']
mixtures = ['food', 'mixes']
...
def getLabelsFromArray(words):
    ...
    # check peppers
    if "pepper" in words:
        index = words.index("pepper")
        if index > 0 and words[index - 1] in otherPeppers:
            labels.remove("vegetable")
            labels.add("spice or herb")
...
# pages for wine pairing
winePages = [("Albarino and Alvarinho", 1703), ("Barbera", 1704), ("Cabernet Sauvignon", 1706), ("Chardonnay", 1707),
             ("Gewurtztraminer", 1710), ("Malbec", 1713), ("Merlot", 1714), ("Pinot Blanc and Pinot Bianco", 1718),
             ("Pinot Noir", 1719), ("Pinot Gris and Pinot Grigio", 1720), ("Riesling", 1723), ("Sangiovese", 1724),
             ("Sauvignon Blanc and Fume Blanc", 1725), ("Syrah and Shiraz", 1729), ("Tempranillo", 1730),
             ("Zinfandel", 1733), ("Bordeaux", 15934)]
...
winepageout = open("winepage.txt", "w")
...
# Get each wine page
for wine in winePages:
    suggestedWine = wine[0]
    wineID = wine[1]
    soupWine = None
    try:
        url = "http://allrecipes.com/recipes/{}".format(wineID)

        with urllib.request.urlopen(url) as response:
            soupWine = BeautifulSoup(response.read(), "html.parser")

    except urllib.error.HTTPError as e:
        outputFile.write("{0}: No wine page".format(wineID))
        outputFile.write(e.reason)
        outputFile.write("\n")
    except urllib.error.URLError as e:
        outputFile.write("{0}: URL ERROR".format(wineID))
        outputFile.write(e.reason)
        outputFile.write("\n")
    except SocketError as e:
        outputFile.write("{0}: SOCKET ERROR".format(wineID))
        outputFile.write("\n")

    print(soupWine)
    if soupWine:
        print("hello")
        recipeLinks = soupWine.find_all("a", class_= "fixed-recipe-card__title-link")
        print(recipeLinks)

        for tag in recipeLinks:
            recipelink = tag['href']
            print(recipelink)
            recipeId = recipelink.split("/")[4]
            time.sleep(1.5)
            try:
                ...
                outputFile.write("{0}: No wine page".format(recipeId))
                outputFile.write(e.reason)
                outputFile.write("\n")
            except urllib.error.URLError as e:
            ...
            if soup:
                ...
                servingSpan = soup.find("meta", id="metaRecipeServings")
                ...
                recipeCategory = soup.find_all("meta", itemprop="recipeCategory")
                #
                # get title
                #
                print(titleSpan)
                ...
                # print(recipeCategory[0]['content'].split(" "))
                cat = recipeCategory[0]['content']
                if len(recipeCategory) == 2:
                    subcat = recipeCategory[1]['content']
                else:
                    subcat = "None"
                # cat = [x for x in recipeCategory[0]['content'].split(" ") if x not in ('and', '&')]
                # subcat = [x for x in recipeCategory[1]['content'].split(" ") if x not in ('and', '&')]
                ...
                # get servings
                #
                servings = servingSpan['content'] if servingSpan is not None else None
